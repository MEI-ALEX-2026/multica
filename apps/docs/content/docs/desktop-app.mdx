...
Multica Desktop is a native desktop app for macOS, Windows, and Linux. For the environment it is configured for, it talks to the same backend as the web app and shows the same data. By default Desktop uses the local self-hosted server at `localhost`; Cloud or another instance can be configured with a local runtime config file. Desktop also adds a few things the browser can't: **independent tab groups per [workspace](/workspaces)**, **automatic [daemon](/daemon-runtimes) startup**, and **one-click upgrades**.
...
<Callout type="info">
**Desktop defaults to the local self-hosted server, but can be pointed at another instance with a local config file.** There is still no in-app connection picker. Desktop reads `~/.multica/desktop.json` before the renderer starts; if the file is missing, it uses `http://localhost:8080` for the API and `http://localhost:3000` for the web app.
